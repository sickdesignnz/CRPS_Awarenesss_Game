...
export class GameFSM {
  ...
  constructor(s: GameState, rng: RNG, hooks: Hooks = {}, cards: Card[]) {
    this.s = s
    this.rng = rng
    this.hooks = hooks
    this.cards = cards
  ...
  drawCard() {
    // Flare check first
    const nextFlare = flareRoll(this.rng, this.s.flare)
    this.s.flare = nextFlare

    if (!nextFlare.free) {
      announce(`Flare turn ${this.s.flare.stuckTurns} of 3 — still stuck`)
      this.s.turn += 1
      this.emit()
      return
    } else {
      announce('Flare cleared — you are free this turn')
    }

    // Deterministic draw from deck
    const card = this.rng.pick(this.cards)
    const outcome = card.outcomes
