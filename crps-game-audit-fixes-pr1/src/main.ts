...
// Public assets are served from /assets at runtime
...
boardImg.src = BOARD_SRC
const cardsImg = new Image()
cardsImg.src = CARDS_SRC
...
function render() {
  ...
  if (boardImg.complete && boardImg.naturalWidth) {
    ...
    // Simple flare glow preview (dampened when reduce motion)
    if (!reduceMotion() && !state.flare.free) {
      const t = (Date.now() % 1000) / 1000
      ctx.save()
      ctx.globalAlpha = 0.25 + 0.25 * Math.sin(t * Math.PI * 2)
      ctx.fillStyle = '#ff8800'
      ctx.fillRect(x, y, w, h) // placeholder: later restrict to tile
      ctx.restore()
    }
  } else {
